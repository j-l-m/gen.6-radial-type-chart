	...
	Not really the kind of use Highcharts.js was inteded for
	...
	*/
	...
	//------pie chart set up-------------------	
			$('#container').highcharts({
				...
				title: {
					text: 'Pokémon Gen.6 Interactive Type Chart'
				},
				subtitle: {
					text: 'Attack (outer circle) and Defense Multiplier (inner circle)'
				},
				tooltip: {
				...
				{//inner pie
					...
					size:"35%",			//size of outer pie
					innerSize: "50%",   //inner size of outer pie
					data:createDefData("Normal")
				},
				...
				{ //middle pie series
					...
					innerSize: "65%",	//inner size of inner pie
...
//---------------------Functions--------------------------------------
		...
		/*
		 Creates the data for Series 1, the middle pie chart
		...
		*/		
		...
		/*
		 Creates the data for Series 0, the inner pie chart
		 and sets the default selected options
		 Returns the data set as an array
		*/		
		function createDefData(type){
			
			var data= [];
			var type_index = type_names.indexOf(type);
			$.each(type_matchups, function(index, value){
						data.push({
							name:value[type_index]+'x',
							y:20,
							color:defColor(value[type_index]),
						});
					});
			return data;		
		}
		...
		/*
		 Assigns the appropriate color to the inner pie slice based on the value of the damage multiplier
		*/
